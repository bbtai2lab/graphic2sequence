...
#   - 在v4的基础上，接着添加一层CNN降size。并且修改大kernel的CNN。(v5)
#-----------------------------------------------------------------------------
...
class AE(torch.nn.Module):
    ...
    def __init__(self, num_latent, in_channels, num_feature):
        ...
        self.encoder = torch.nn.Sequential(
                    ...
                    torch.nn.Conv2d(num_feature, num_feature*2, kernel_size=3, stride=3, padding=1, bias=False), # 38
                    torch.nn.BatchNorm2d(num_feature*2),
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.Conv2d(num_feature*2, num_feature*3, kernel_size=3, stride=3, padding=2, bias=False), # 14
                    torch.nn.BatchNorm2d(num_feature*3),
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.Conv2d(num_feature*3, num_feature*3, kernel_size=3, stride=3, padding=2, bias=False), # 6
                    torch.nn.BatchNorm2d(num_feature*3),
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.Conv2d(num_feature*3, num_feature*3, kernel_size=2, stride=2, padding=0, bias=False), # 3
                    torch.nn.BatchNorm2d(num_feature*3),
                    torch.nn.LeakyReLU(0.2, inplace=True),
                    )

        self.latent = torch.nn.Sequential(
                    torch.nn.Linear(num_feature*3*3*3, num_latent, bias=False),
                    torch.nn.LeakyReLU(0.2, inplace=True),
                    torch.nn.Linear(num_latent, num_feature*3*3*3, bias=False),
                    torch.nn.LeakyReLU(0.2, inplace=True),
                    )

        self.decoder = torch.nn.Sequential(
                    torch.nn.ConvTranspose2d(num_feature*3, num_feature*3, kernel_size=2, stride=2, padding=0, bias=False), # 6
                    torch.nn.BatchNorm2d(num_feature*3),
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.ConvTranspose2d(num_feature*3, num_feature*3, kernel_size=3, stride=3, padding=2, bias=False), # 14
                    torch.nn.BatchNorm2d(num_feature*3),
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.ConvTranspose2d(num_feature*3, num_feature*2, kernel_size=3, stride=3, padding=2, bias=False), # 38
                    torch.nn.BatchNorm2d(num_feature*2),
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.ConvTranspose2d(num_feature*2, num_feature, kernel_size=3, stride=3, padding=1, bias=False), # 112
                    torch.nn.BatchNorm2d(num_feature),
                    torch.nn.LeakyReLU(0.2, inplace=True),
...
if __name__ == "__main__":
    ...
    info = """
    AE generative model on bootstrap images (version 5)
    torch.nn.Conv2d(in_channels, num_feature, kernel_size=4, stride=2, padding=1, bias=False), # 112
    torch.nn.Conv2d(num_feature, num_feature*2, kernel_size=3, stride=3, padding=1, bias=False), # 38
    torch.nn.Conv2d(num_feature*2, num_feature*3, kernel_size=3, stride=3, padding=2, bias=False), # 14
    torch.nn.Conv2d(num_feature*3, num_feature*3, kernel_size=3, stride=3, padding=2, bias=False), # 6
