...
def train(encoder, decoder, train_loader, test_loader, criterion, optimizer,
    ...
    for epoch in range(num_epochs):
        ...
        if (epoch+1) % save_interval == 0 or epoch==0:
            ...
            torch.save(encoder.fc, os.path.join("./Logs/{}/".format(time), "encoder.fc_epoch{}.pkl".format(epoch)) )
            torch.save(encoder.bn, os.path.join("./Logs/{}/".format(time), "encoder.bn_epoch{}.pkl".format(epoch)) )
...
if __name__ == "__main__":
    ...
    # information
    info = "CNN model    : resnet152, fixed=True \n"
    info+= "num_epochs   : {} \n".format(num_epochs)
    info+= "batch_size   : {} \n".format(batch_size)
    info+= "log_interval : {} \n".format(log_interval)
