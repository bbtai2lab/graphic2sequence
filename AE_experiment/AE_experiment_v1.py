...
class AE(torch.nn.Module):
    ...
    def __init__(self, num_latent, in_channels, num_feature):
        ...
        self.encoder = torch.nn.Sequential(
                    torch.nn.Conv2d(in_channels, 1, kernel_size=1, stride=1, padding=0), # 224
                    torch.nn.BatchNorm2d(1),
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.Conv2d(1, num_feature, kernel_size=4, stride=2, padding=1), # 112
                    torch.nn.BatchNorm2d(num_feature),
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.Conv2d(num_feature, num_feature*2, kernel_size=6, stride=4, padding=1), # 28
        ...
        self.decoder = torch.nn.Sequential(
                    torch.nn.ConvTranspose2d(num_feature*2, num_feature, kernel_size=6, stride=4, padding=1), # 112
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.ConvTranspose2d(num_feature, 1, kernel_size=4, stride=2, padding=1), # 224
                    torch.nn.LeakyReLU(0.2, inplace=True),

                    torch.nn.ConvTranspose2d(1, in_channels, kernel_size=1, stride=1, padding=0), # 224
                    torch.nn.Sigmoid(),
                    )
